...
// first time thru	
	...
	if ( is_null($head_string) )  {
		...
		$appnts = new AppointMents();
		$no_of_forms = $appnts->count_appoint_ments($dpgconn, $user_id);
		if ($no_of_forms > 0)  {
			build_pc_array($no_of_forms, $sys_lim);
			display_rows($dpgconn, $user_id, $maint_go, $say_yes, $say_non);
		}  else  {
			echo '<p class="error">There are no Appointments in the database for this User.</p>';
		}
	}
}
...
function display_rows ($dbc, $user_id, $maint_go, $say_yes, $say_non)  {
	if ( (isset($_SESSION['pcarray']))   )  {
		...
		if ( (is_array($pc_array))  )  {
			...
			$lim_it    = $pc_array['lim'];
			build_display_line_am ($dbc, $user_id, $lim_it, $off_set, $maint_go, $say_yes, $say_non);
